...
import { AuditErrorState } from "./AuditEmptyState";
...
export const AuditLogsTable: React.FC = () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const transformedLogs = logsData.map((item) => ({
          ...
          item: isOrgAccount
            ? `${item.subject_type}${
                item.org?.name ? ` in ${item.org.name}` : ""
              }`
            : item.subject_type,
  ...
  useEffect(() => {
    const hasEmptyFilters = Object.values(filters).every(
      (value) => value === ""
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && logs.length === 0 && (
        <AuditEmptyState isOrgAccount={isOrgAccount} />
      )}
    </div>
  );
};
